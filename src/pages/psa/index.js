...
import Psapopup from "@/components/common/Psapopup";
...
export default function WARInitiator() {
  ...
  const [personnelActionIsInitiator, setpersonnelActionIsInitiator] =
    useState(1);
  ...
  const [personnelActionIsSuperadmin, setpersonnelActionIsSuperadmin] =
    useState(0);
  ...
  const [
    personalActioninitiatorReportList,
    setPersonalActioninitiatorReportList,
  ] = useState();
  ...
  useEffect(() => {
    let loggedUserRole = window.localStorage.getItem("loggedFirstName");
    console.log(4444, loggedUserRole);

    setLoggedUserRole(loggedUserRole.trim());
  }, []);
  ...
  const [weeklyAbsenceReportSubmitted, setWeeklyAbsenceReportSubmitted] =
    useState();
  const [weeklyAbsenceReportApproved, setWeeklyAbsenceReportApproved] =
    useState();
  const [weeklyAbsenceReportRejected, setWeeklyAbsenceReportRejected] =
    useState();
  const [weeklyAbsenceReportPending, setWeeklyAbsenceReportPending] =
    useState();
  const [submitCount, setSubmitCount] = useState(0);
  const [approvedCount, setApprovedCount] = useState(0);
  const [rejectCount, setRejectCount] = useState(0);
  const [
    pendingForApprovalCountForReqReportsCount,
    setPendingForApprovalCountForReqReports,
  ] = useState(0);
  const [approvedCountForReqReportsCount, setApprovedCountForReqReports] =
    useState(0);
  const [rejectCountForReqReportsCount, setRejectCountForReqReports] =
    useState(0);
  ...
  const [statuses] = useState([
    "Pending",
    "Pending for Approval Site Admin/ Dept Head",
    "Pending for Approval Budget Clerk",
    "Pending for Approval Budget Manager",
    "Pending for Approval Educational Services Executive Secretary",
    "Pending for Approval Elementary Education Exe.Secretary",
    "Pending for Approval Secondary Education Exe.Secretary",
    "Pending for Approval Executive Management",
    "Pending for Approval HR Executive Secretary",
    "Pending for Approval HR Technician",
    "Pending for Approval HR Technician (Payroll)",
    "Closed By HR Technician",
    "Rejected",
  ]);
  const [statusesPayrolls] = useState([
    "Acknowledged",
  ...
  const approverLevelTypeDic = {
    0: "Initiator",
    1: "Site Admin/ Dept Head",
    2: "Budget Clerk",
    3: "Budget Manager",
    4: "Educational Services Executive Secretary",
  ...
  const [approverHrexectivesecuserId, setapproverHrexectivesecuserId] =
    useState({});
  ...
  const [InitiateReportsfullscreen, setinitiateReportsfullscreen] =
    useState(false);
  ...
  const [requestApprovalConfirmVisible, setRequestApprovalConfirmVisible] =
    useState(false);
  ...
  const getApproverLevelOptions = async () => {
    ...
    let aprroverLevelName = levelOption?.filter(
      (item) => item["name"] == approverLevelTypeDic[approverArray.length - 1]
    );
    setFallBackApproverLevel({
      code: aprroverLevelName?.[0]?.code,
      name: approverLevelTypeDic[approverArray.length - 1],
    });
    let initialfallBackApprover = approverOption?.filter(
      (item) => item["label"] == approverLevelTypeDic[approverArray.length - 1]
    );
    setFallBackApprover(initialfallBackApprover?.[0]?.value);
  };
  const uploadSignature = async (signature) => {
    const fileName =
      "profile/" + loggedInUserData?.userId ?? loggedUserId + "/signature.png";
  ...
  const loadSignatureFile = async (filePath) => {
    let file = null;
    try {
      await Storage.get(filePath).then((result) => {
        //console.log(result);
        file = result;
      });
  ...
  const handleAddSignature = async () => {
    ...
    await cognito.listUsers(params, (err, data) => {
      if (data) {
        let signature = data.Users[0].Attributes.find(
          (attr) => attr.Name === "custom:userSignature"
        )?.Value;
  ...
  const proccessedApproverOptions = useMemo(() => {
    const options = approversOptions?.filter(
      (item) => item["label"] == fallBackApproverLevel.code
    );
  ...
  const onClickRequestedReportEditPopUp = async (requestData) => {
    ...
    users.forEach((user, i) => {
      ...
      userCode = user.Attributes.find(
        (attr) => attr.Name === "custom:userCode"
      )?.Value;
    ...
    try {
      const particularReportMetaData = await fetchInitiateReport(
        requestData.id
      );
    ...
    try {
      const particularReportTransactionCycleResponse =
      ...
      if (particularReportTransactionCycleResponse) {
        ...
        for (
          let i = 0;
          i < particularReportTransactionCycleResponse.length;
          i++
        ) {
          let empName = "",
            empCode = "";
          ...
          for (let j = 0; j < newRes.length; j++) {
            ...
            if (
              currentItem.userId ==
              particularReportTransactionCycleResponse[i].to_Employee_Id
            ) {
    ...
    try {
      const particularReportResponse = await fetchInitiateReport(
        requestData.id
      );
    ...
    try {
      ...
      if (PersonnelActionTransactionCycle.length >= 3) {
        ...
        for (let i = 0; i < 3; i++) {
          if (
            PersonnelActionTransactionCycleLate[i].status ===
            "ReviewedResubmitted"
          ) {
            if (i === 2) {
  ...
  const RemainderFunction = async (rowData) => {
    ...
    const reportDetails = await buildReportDetails(
      result?.data?.getTransactionCyclePersonnel,
      rowData.formTitle,
      "Open",
      rowData.initiator,
      rowData.deptHead,
      loggedUserId,
      result?.data?.getTransactionCyclePersonnel?.to_Employee_Id
    );
    ...
    await handleNotificationsOnStatusChangeforPersonnelAction(
      "REMINDER",
      reportDetails,
  ...
  const selectedEmployeeDetails = async (requestData) => {
    ...
    try {
      let particularReportTransactionCycleResponse =
        await fetchPaginatedRecords(
          queries.listTransactionCyclePersonnels,
          { report_Id: { eq: requestData.id } },
          { field: "createdAt", direction: "desc" },
      ...
      particularReportTransactionCycleResponse =
        particularReportTransactionCycleResponse.reverse();
        ...
        for (
          ...
          for (let j = 0; j < newRes.length; j++) {
            ...
            if (
              ...
              let extraUser = await fetchUser(
                particularReportTransactionCycleResponse[i].to_Employee_Id
              );
              empName =
                extraUser?.UserAttributes?.find((attr) => attr.Name === "name")
                  ?.Value || "";
              empCode =
                extraUser?.UserAttributes?.find(
                  (attr) => attr.Name === "custom:userCode"
                )?.Value || "";
            }
          }
          ...
          const inputDateString =
            particularReportTransactionCycleResponse[i].createdAt;
          ...
          let secondApprover = requestData.budgetClerk
            ? requestData.budgetClerk
            : 0;
          let thirdApprover = requestData.budgetManager
            ? requestData.budgetManager
            : 0;
          let executivedirelemId = requestData.executivedirelemId
            ? requestData.executivedirelemId
            : 0;
          let elemorsec = requestData.assignelemorsec
            ? requestData.assignelemorsec
            : 0;
          ...
          let fourthApprover = requestData.executiveSecretary
            ? requestData.executiveSecretary
            : 0;
          ...
          if (i == 0) {
            ...
            let userFirstName1 = await GetValueFromArray(
              initiatorUsers.UserAttributes,
              "custom:firstName"
            );
            let userLastName1 = await GetValueFromArray(
              initiatorUsers.UserAttributes,
              "custom:lastName"
            );
            let userCodeEmp1 = await GetValueFromArray(
              initiatorUsers.UserAttributes,
              "custom:userCode"
            );
            ...
            let obj = {
              ...
              appStatus: "Submitted",
              approvedDate: particularReportTransactionCycleResponse[i]
                .createdAt
                ? handleDateSelect(
                    particularReportTransactionCycleResponse[i].createdAt
                  )
          ...
          //form history remark
          if (
            particularReportTransactionCycleResponse[i].status === "Rejected"
          ) {
            let obj = {
              empName: empName,
              remarkType: "Rejected",
      ...
      <div className="flex justify-center items-center w-full gap-2">
        ...
        <div className="flex justify-center w-full gap-2">
          ...
          {(row.status === "Submitted" &&
            row.pretexts != "Pending for Approval" &&
            row.pstatus != "Closed") ||
          row.status === "Rejected_Editable" ? (
            <Link
              href="#"
            ...
            <Link href="#" className="py-2 px-5"></Link>
          )}
        </div>
      </div>
    );
  ...
  const onClickPopup = async (requestData) => {
    ...
    let fromDate = moment(requestData.from_date, "YYYY-MM-DD").format(
      "MM/DD/YYYY"
    );
    ...
    if (requestData.id) {
      let newResponse = [];
      var employeeResponses = await DataStore.query(
        InitiateClassifiedWeeklyAbsenceReportEmployee,
        (c) => c.initiate_classified_weekly_absence_report_id.eq(requestData.id)
      );
      if (employeeResponses.length > 0) {
        for (let index = 0; index < employeeResponses.length; index++) {
          ...
          var employeeDetailsResponses = await DataStore.query(
            InitiateClassifiedWeeklyAbsenceReportEmployeeDetails,
            (c) =>
              c.initiate_classified_weekly_absence_report_employee_id.eq(
                item.id
              )
          );
          ...
          for (let i = 0; i < employeeDetailsResponses.length; i++) {
            ...
            if (detailItem.substitute_emp_id) {
              let substituteEmployeeData = await getEmployeeResponse(
                detailItem.substitute_emp_id
              );
            ...
            let absentCodeName = "";
            let absentCodeResponses = await DataStore.query(
              AbsentCode,
              detailItem.absent_code_id
            );
            ...
            let absent_date = moment(detailItem.absent_date).format(
              "MM/DD/YYYY"
            );
  ...
  const getName = async (id) => {
    ...
    let name = userdetails?.UserAttributes?.find(
  ...
  const exportFilteredDataToCSV = async (fileName) => {
      ...
      dataToExport = await Promise.all(
        dataToExport.map(async (obj, i) => {
          ...
          return {
            ...
            exedirelemId,
            approverHrexectivesec:
              psayesorno == null ? "-" : approverHrexectivesec,
            siNo: i + 1,
            psayesno: psayesorno == null ? "NO" : "YES",
            elemEdu:
              obj.assignelemorsec == "Elementery" ? executivedirelemId : null,
            secEdu:
              obj.assignelemorsec == "Secondary" ? executivedirelemId : null,
          };
        })
      );
    ...
    if (InitiateReportssidebar == true || InitiateReportsfullscreen == true) {
      ...
      dataToExport = dataToExport.flatMap((item) =>
        JSON.parse(item.employeeName).map((employeeName) => ({
          ...item,
    ...
    const columnMappings = [
      ...
      {
        name: "Ed Services Executive Secretary",
        property: "executiveSecretary",
      },
      { name: "Elementary Education", property: "elemEdu" },
      { name: "Secendory Education", property: "secEdu" },
      {
        name: "Executive Director of Elementary Education",
        property: "exedirelemId",
      },
  ...
  const getUserResponse = (id) => {
    ...
    let firstName = GetValueFromArray(
      getEmployeeResponse[0].Attributes,
      "custom:firstName"
    );
    let lastName = GetValueFromArray(
      getEmployeeResponse[0].Attributes,
      "custom:lastName"
    );
  ...
  const loadAbsenceReport = async () => {
    ...
    var weeklyAbsenceReport = await DataStore.query(
      InitiateClassifiedWeeklyAbsenceReport,
      Predicates.ALL,
      {
    ...
    for (let i = 0; i < weeklyAbsenceReport.length; i++) {
      ...
      if (weeklyAbsenceReport[i].from_date && weeklyAbsenceReport[i].to_date) {
        let fromDate = moment(
          weeklyAbsenceReport[i].from_date,
          "YYYY-MM-DD"
        ).format("MM/DD/YYYY");
        let toDate = moment(
          weeklyAbsenceReport[i].to_date,
          "YYYY-MM-DD"
        ).format("MM/DD/YYYY");
      ...
      if (
        ...
        weeklyAbsenceReport[i].status_date_time !== undefined
      ) {
        submittedOn =
          weeklyAbsenceReport[i].status_date_time !== null
            ? moment(weeklyAbsenceReport[i].status_date_time).format(
                "MM/DD/YYYY"
              )
            : "-";
      } else {
      ...
      // //*School Name
      if (
        weeklyAbsenceReport[i].school_id !== null &&
        weeklyAbsenceReport[i].school_id !== undefined
      ) {
        var schoolResponses = await DataStore.query(Schools, (c) =>
          c.id.eq(weeklyAbsenceReport[i].school_id)
        );
      ...
      if (
        ...
        weeklyAbsenceReport[i].payroll_status ===
          InitiateClassifiedWeeklyAbsenceReportStatus.PAYROLL_ACCEPTED
      ) {
        status = "Approved";
        remark = "-";
      } else if (
        weeklyAbsenceReport[i].payroll_status !== null &&
        weeklyAbsenceReport[i].payroll_status ===
          InitiateClassifiedWeeklyAbsenceReportStatus.PAYROLL_REJECTED
      ) {
        status = "Rejected";
        remark = weeklyAbsenceReport[i].payroll_remark;
      } else if (
        weeklyAbsenceReport[i].approver_status !== null &&
        weeklyAbsenceReport[i].approver_status ===
          InitiateClassifiedWeeklyAbsenceReportStatus.APPROVAL_ACCEPTED
      ) {
        status = "Approved";
        remark = "-";
      } else if (
        weeklyAbsenceReport[i].approver_status !== null &&
        weeklyAbsenceReport[i].approver_status ===
          InitiateClassifiedWeeklyAbsenceReportStatus.APPROVAL_REJECTED
      ) {
        status = "Rejected";
        ...
        weeklyAbsenceReport[i].payroll_status === null &&
        weeklyAbsenceReport[i].approver_status === null &&
        weeklyAbsenceReport[i].status ===
          InitiateClassifiedWeeklyAbsenceReportStatus.SUBMITTED
      ) {
        status = "Submitted";
        remark = "-";
      } else if (
        ...
        weeklyAbsenceReport[i].approver_status === null &&
        weeklyAbsenceReport[i].status ===
          InitiateClassifiedWeeklyAbsenceReportStatus.OPEN
      ) {
        status = "Pending";
        remark = "-";
      }
      ...
      newResponse.push({
        ...
        payroll_remark:
          weeklyAbsenceReport[i].payroll_remark !== null
            ? weeklyAbsenceReport[i].payroll_remark
            : "-",
      ...
      if (i === weeklyAbsenceReport.length - 1) {
        let weeklyAbsenceReportForInitiator = newResponse.filter(
          (obj) => obj.user_id === loggedUserId
        );
        ...
        //SubmittedCount
        const submittedCount = weeklyAbsenceReportForInitiator.filter(
          (obj) => obj.status === "Submitted"
        );
        setWeeklyAbsenceReportSubmitted(submittedCount.length);

        //SubmittedCount
        const approvedCount = weeklyAbsenceReportForInitiator.filter(
          (obj) => obj.status === "Approved"
        );
        setWeeklyAbsenceReportApproved(approvedCount.length);

        //RejectedCount
        const rejectedCount = weeklyAbsenceReportForInitiator.filter(
          (obj) => obj.status === "Rejected"
        );
        setWeeklyAbsenceReportRejected(rejectedCount.length);

        //RejectedCount
        const pendingCount = weeklyAbsenceReportForInitiator.filter(
  ...
  const loadReportLogs = async (reportId) => {
    ...
    var loadReportLogsResponse = await DataStore.query(ReportsLog, (c) =>
      c.report_id.eq(reportId)
    );
    if (loadReportLogsResponse.length > 0) {
      ...
      for (let i = 0; i < loadReportLogsResponse.length > 0; i++) {
        ...
        let name;
        let ProfilePic;
        let dateTime = moment(loadReportLogsResponse[i].date_and_time).format(
          "HH:mm"
        );
        //*Get user
        ...
        if (userId) {
          try {
            ...
            let profilePath = GetValueFromArray(
              exist.UserAttributes,
              "picture"
            );
  ...
  const checkApplicationAccessibility = async () => {
    //personnel - Action
    let personnelActionIsInitiator = reactLocalStorage.get(
      "personnelIsInitiator"
    );
    //console.log('personnelActionIsInitiator --->', personnelActionIsInitiator);
    let personnelActionIsApprover = reactLocalStorage.get(
      "personnelIsApprover"
    );
    let personnelActionIsPayroll = reactLocalStorage.get("personnelIsPayroll");
    let personnelActionIsSuperadmin = reactLocalStorage.get(
      "personnelActionIsSuperadmin"
    );
  ...
  useEffect(() => {
    ...
    // onLoad();
    // loadUsers()
    MyAccountTable.getProductsSmall().then((data) =>
  ...
  const getFirstAndLastName = (attributes) => {
    if (attributes) {
      const fnameAttribute = attributes.find(
        (attr) => attr.Name === "custom:firstName"
      );
      const lnameAttribute = attributes.find(
  ...
  function getUserMap(user) {
    ...
    const userEmailAttribute = user.Attributes.find(
      (attr) => attr.Name === "email"
    );
    const userCodeAttribute = user.Attributes.find(
      (attr) => attr.Name === "custom:userCode"
    );
    const signatureAttribute = user.Attributes.find(
      (attr) => attr.Name === "custom:userSignature"
    );
    const userRoleAttribute = user.Attributes.find(
      (attr) => attr.Name === "custom:role"
    );
    const userReportingManager = user.Attributes.find(
      (attr) => attr.Name === "custom:SPReportingManager"
    )?.Value;
    return {
      userId: sub?.Value,
  ...
  async function fetchPersonnelActionInitiatorReportResponse(
    isSuperadmin,
    userId
  ) {
    // const filter = {
  ...
  const fetchInitiateReport = async (id) => {
    try {
      const result = await API.graphql(
        graphqlOperation(queries.getPersonnelActionInitiatorForm, { id: id })
      );
  ...
  const initiatorBindList = async () => {
    ...
    try {
      let PersonalActionFormResponse =
        await fetchPersonnelActionInitiatorReportResponse(
          personnelActionIsSuperadmin,
          loggedUserId
        );
      ...
      if (PersonalActionFormResponse) {
        ...
        let approvedCount = 0;
        let rejectCount = 0;
        PersonalActionFormResponse = PersonalActionFormResponse.sort(
        ...
        try {
          for (let i = 0; i < PersonalActionFormResponse.length; i++) {
            ...
            let fromDate = moment(
              currentItem.effectiveDatesFrom,
              "YYYY-MM-DD"
            ).format("MM/DD/YYYY");
            let toDate = moment(
              currentItem.effectiveDatesTo,
              "YYYY-MM-DD"
            ).format("MM/DD/YYYY");
            ...
            const TransactionCyclePersonnelResponse =
              await fetchReportTransactionCycles(currentItem?.id);

            for (let i = 0; i < TransactionCyclePersonnelResponse.length; i++) {
              if (
                TransactionCyclePersonnelResponse[
                  TransactionCyclePersonnelResponse.length - 1
                ].remark
              ) {
                if (
                  TransactionCyclePersonnelResponse[
                    TransactionCyclePersonnelResponse.length - 1
                  ].approver_level == loggedUserId
                ) {
                  remark =
                    TransactionCyclePersonnelResponse[
                      TransactionCyclePersonnelResponse.length - 1
            ...
            for (let i = 0; i < TransactionCyclePersonnelResponse.length; i++) {
              if (
                TransactionCyclePersonnelResponse[i].status ===
                "ReviewedResubmitted"
              ) {
            ...
            let secondApprover = currentItem.budgetClerk
              ? currentItem.budgetClerk
              : 0;
            let thirdApprover = currentItem.budgetManager
              ? currentItem.budgetManager
              : 0;
            let executivedirelemId = currentItem.executivedirelemId
              ? currentItem.executivedirelemId
              : 0;
            let elemorsec = currentItem.assignelemorsec
              ? currentItem.assignelemorsec
              : 0;
            let psayesorno = currentItem.psayesorno
              ? currentItem.psayesorno
              : 0;
            let teacher = currentItem.deptHead ? currentItem.deptHead : 0;
            let payroll = payrolluserId?.userId;
            let exedirelemId = exedirelemIduserId?.userId;
            let approverHrexectivesec = approverHrexectivesecuserId?.userId;
            let fourthApprover = currentItem.executiveSecretary
              ? currentItem.executiveSecretary
              : 0;
            let fifthApprover = currentItem?.executiveManagement;
            let sixthApprover = currentItem?.hrTechnician;
            let TransactionCyclePersonnelResponseLate =
              await fetchReportTransactionCycles(currentItem.id);
            ...
            try {
              if (toEmployeeId != null && pstatus != "Closed") {
                ...
                employeeResponses = employeeResponses.filter(
                  (item) => item.user_Id == toEmployeeId
                );
            ...
            try {
              ...
              let fname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:firstName"
              );
              let lname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:lastName"
              );
              if (fname) {
            ...
            try {
              let x = users.filter(
                (u) => u.Username === currentItem.submited_by
              );
              let fname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:firstName"
              );
      ...
      TransactionCyclePersonnelResponse = groupBy(
        TransactionCyclePersonnelResponse,
        "report_Id"
      );
      if (TransactionCyclePersonnelResponse) {
        ...
        for (let report_Id in TransactionCyclePersonnelResponse) {
          ...
          const PersonalActionFormResponse = await fetchInitiateReport(
            report_Id
          );

          //check if report exists
          if (
            PersonalActionFormResponse &&
            PersonalActionFormResponse.submited_by
          ) {
            const TransactionCyclePersonnelResponseNew =
              await fetchReportTransactionCycles(report_Id);

            let fromDate = moment(
              PersonalActionFormResponse.effectiveDatesFrom,
              "YYYY-MM-DD"
            ).format("MM/DD/YYYY");
            let toDate = moment(
              PersonalActionFormResponse.effectiveDatesTo,
              "YYYY-MM-DD"
            ).format("MM/DD/YYYY");

            const period = `${fromDate} to ${toDate}`;
            let status = currentItem.isApproved
              ? "Approved"
              : currentItem.remark
              ? "Rejected"
              : "Pending for Approval";
            ...
            if (TransactionCyclePersonnelResponseNew.length) {
              status =
                TransactionCyclePersonnelResponseNew[0].remark &&
                TransactionCyclePersonnelResponseNew[0].status === "Rejected"
                  ? TransactionCyclePersonnelResponseNew[0].approver_level ==
                    loggedUserId
                    ? "Rejected_Editable"
                    : "Rejected"
                  : status;
            ...
            let secondApprover = PersonalActionFormResponse.budgetClerk
              ? PersonalActionFormResponse.budgetClerk
              : 0;
            let thirdApprover = PersonalActionFormResponse.budgetManager
              ? PersonalActionFormResponse.budgetManager
              : 0;
            let executivedirelemId =
              PersonalActionFormResponse.executivedirelemId
                ? PersonalActionFormResponse.executivedirelemId
                : 0;
            let elemorsec = PersonalActionFormResponse.assignelemorsec
              ? PersonalActionFormResponse.assignelemorsec
              : 0;
            let teacher = PersonalActionFormResponse.deptHead
              ? PersonalActionFormResponse.deptHead
              : 0;
            let payroll = payrolluserId?.userId;
            let psayesorno = PersonalActionFormResponse.psayesorno
              ? PersonalActionFormResponse.psayesorno
              : 0;
            let fourthApprover = PersonalActionFormResponse.executiveSecretary
              ? PersonalActionFormResponse.executiveSecretary
              : 0;
            ...
            let approverHrexectivesec = approverHrexectivesecuserId?.userId;
            let TransactionCyclePersonnelResponseLate =
              await fetchReportTransactionCycles(PersonalActionFormResponse.id);
            ...
            try {
              let x = users.filter(
                (u) => u.Username === PersonalActionFormResponse.deptHead
              );
              let fname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:firstName"
              );
            ...
            try {
              let x = users.filter(
                (u) => u.Username === PersonalActionFormResponse.created_by
              );
              let fname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:firstName"
              );
            ...
            let obj = {
              ...
              executiveSecretary:
                PersonalActionFormResponse?.executiveSecretary,
              executiveManagement:
                PersonalActionFormResponse?.executiveManagement,
              executivedirelemId:
                PersonalActionFormResponse?.executivedirelemId,
        ...
        const approveCount = approvalFinalData.filter(
          (obj) => obj.status === "Approved"
        );

        setApprovedCountForReqReports(approveCount.length);

        const rejectedCount = approvalFinalData.filter(
          (obj) => obj.status === "Rejected"
        );
        setRejectCountForReqReports(rejectedCount.length);

        const pendingForApproverCount = approvalFinalData.filter(
          (obj) => obj.status === "Pending for Approval"
        );
      ...
      const TransactionCyclePersonnelResponseForPayroll =
        personnelActionIsSuperadmin
          ? await DataStore.query(TransactionCyclePersonnel, (c) => c, {
      ...
      if (TransactionCyclePersonnelResponseForPayroll) {
        ...
        for (
          let i = 0;
          i < TransactionCyclePersonnelResponseForPayroll.length;
          i++
        ) {
          let currentItem = TransactionCyclePersonnelResponseForPayroll[i];
          const PersonnelActionInitiatorFormReportResponse =
            await fetchInitiateReport(currentItem.report_Id);
          ...
          if (PersonnelActionInitiatorFormReportResponse.length > 0) {
            let fromDate = moment(
              PersonnelActionInitiatorFormReportResponse.from_date,
              "YYYY-MM-DD"
            ).format("MM/DD/YYYY");
            let toDate = moment(
              PersonnelActionInitiatorFormReportResponse.to_date,
              "YYYY-MM-DD"
            ).format("MM/DD/YYYY");
            const period = `${fromDate} to ${toDate}`;
            let status =
            ...
            try {
              let x = users.filter(
                (u) =>
                  u.Username ===
                  PersonnelActionInitiatorFormReportResponse.deptHead
              );
              let fname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:firstName"
              );
            ...
            try {
              let x = users.filter(
                (u) =>
                  u.Username ===
                  PersonnelActionInitiatorFormReportResponse.created_by
              );
              let fname = x[0]?.Attributes.filter(
                (u) => u?.Name === "custom:firstName"
              );
            ...
            let obj = {
              ...
              formTitle:
                PersonnelActionInitiatorFormReportResponse?.formTitle || "",
              boardMeetingDate:
                PersonnelActionInitiatorFormReportResponse?.boardMeetingDate,
              location:
                PersonnelActionInitiatorFormReportResponse?.location || "",
              actionToBeTaken:
                PersonnelActionInitiatorFormReportResponse?.actionToBeTaken,
              type: PersonnelActionInitiatorFormReportResponse?.type,
              personalReportNo:
                PersonnelActionInitiatorFormReportResponse?.personalReportNo,
              deptHead: PersonnelActionInitiatorFormReportResponse?.deptHead,
              budgetClerk:
                PersonnelActionInitiatorFormReportResponse?.budgetClerk,
              budgetManager:
                PersonnelActionInitiatorFormReportResponse?.budgetManager,
              executiveSecretary:
                PersonnelActionInitiatorFormReportResponse?.executiveSecretary,
              executiveManagement:
                PersonnelActionInitiatorFormReportResponse?.executiveManagement,
              hrTechnician:
                PersonnelActionInitiatorFormReportResponse?.hrTechnician,
              status: status,
              remark: remark,
  ...
  async function fetchAllDetailsAndSetObjects(
    target_user_id,
    approver_id,
    next_approver_id,
    initiator_id
    ...
    try {
      const [targetData, approverData, nextApproverData, initiatorData] =
        await Promise.all([
          targetPromise,
          approverPromise,
          nextApproverPromise,
  ...
  const approvalStatusBody = (product) => {
    return (
      <Tag
        value={product.appStatus}
        severity={approveStatusOptions(product)}
      ></Tag>
    );
  };
  ...
  const DatatableValue = [
    {
      ...
      reportno: "Ack test",
      title: "11/22/2023",
      location: " Acknowledged by HR Executive Secretary",
      action: "",
  ...
  const iconHandler = () => {
    ...
    return (
      <div className="flex justify-center items-center w-full gap-2">
        <div className="flex justify-center w-full gap-2">
          <i
            onClick={() => setinitiateReportssidebar(true)}
            className="gusd-eye text-[#667085] text-[20px] font-meduim"
          ></i>
  ...
  const buildReportDetails = async (
    ...
    try {
      const details = await fetchAllDetailsAndSetObjects(
        target_user_id,
        approver_id,
        next_approver_id,
        initiator_id
    ...
    try {
      ...
      if (PersonalActionFormResponse) {
        ...
        if (original.length > 1) {
          let PersonnelActionFormTransactionCycleResponse =
            await fetchReportTransactionCycles(reportId);
          //check for review resubmitted  only once for teacher   shailesh

          if (
            PersonnelActionFormTransactionCycleResponse.length > 1 &&
            PersonnelActionFormTransactionCycleResponse[0].status ===
              "ReviewedResubmitted"
          ) {
            await API.graphql({
              ...
              variables: {
                input: {
                  ...
                  _version:
                    PersonnelActionFormTransactionCycleResponse[0]._version,
                },
              },
            }).then(async () => {
              ...
              if (addNewRowForRMReasponse) {
                ...
                // setCheckedSignature(false);
                const reportDetails = await buildReportDetails(
                  addNewRowForRMReasponse?.data
                    ?.createTransactionCyclePersonnel,
                  PersonalActionFormResponse.formTitle,
                  "Open",
                  PersonalActionFormResponse.submited_by,
                  PersonalActionFormResponse.deptHead,
                  loggedUserId,
            ...
            //console.log('teacher: ', teacher);
            let secondApprover =
              PersonalActionFormResponse?.budgetClerk || null;
            //console.log('secondApprover: ', secondApprover);
            let thirdApprover =
              PersonalActionFormResponse?.budgetManager || null;
            //console.log('thirdApprover: ', thirdApprover);
            let fourthApprover =
              PersonalActionFormResponse?.executiveSecretary || null;
            //console.log('fourthApprover: ', fourthApprover);
            let elemorseceducationapprover =
              macthedemail?.userId ||
              PersonalActionFormResponse?.executivedirelemId;
          ...
          await API.graphql({
            ...
            const reportDetails = await buildReportDetails(
              addNewRowForRMReasponse?.data?.createTransactionCyclePersonnel,
              PersonalActionFormResponse.formTitle,
              "Approved",
              PersonalActionFormResponse.submited_by,
              PersonalActionFormResponse.deptHead,
      ...
      console.log(
        "approveSelectedReport function while fetch report metadata",
        err
      );
    }
  };
  ...
  const rejectSelectedReport = async (reportId) => {
    ...
    try {
      ...
      await API.graphql({
        ...
        const reportDetails = await buildReportDetails(
          responseRow,
          reportData.formTitle,
          "Rejected",
          reportData.submited_by,
          reportData.deptHead,
          loggedUserId,
          fallBackApprover
        );
  ...
  const acknowledgeSelectedReport = async (reportId) => {
    ...
    const reportData = await DataStore.query(
    ...
    try {
      ...
      if (original.length > 0) {
        ...
        await DataStore.save(
          ...
          await handleNotificationsOnStatusChange(
            "Acknowledge",
            reportDetails,
            "PERSONNEL_ACTION_FORM"
          );
  ...
  const handleDeleteRow = async (employeeId) => {
    ...
    const updatedData = personalActionReportList.filter(
  ...
  const footerContentForReminder = (
    <div className="text-center">
      <div className="d-flex">
        <button
          onClick={() => {
            RemainderFunction(ReminderData);
          }}
          type="button"
          className="inline-flex justify-center py-[12px] xl:py-[0.625vw] px-[16px] xl:px-[0.833vw] text-[#2D5BE5] text-[14px] xl:text-[0.729vw] font-medium bg-[#EFF8FF] border border-[#EFF8FF] rounded-md"
          // id="add-family-cancel-btn"
  ...
  const payrollStatusBody = (product) => {
    return (
      <>
        <Badge value="" severity={payrollStatusOptions(product)}></Badge>{" "}
        {product.status}
      </>
    );
  };
  ...
  const requestReportStatus = (product) => {
    ...
    return (
      <>
        <Badge
          value={""}
          severity={requestReportStatusOptions(product)}
        ></Badge>{" "}
        {product.pretexts}
      </>
    );
  ...
  useEffect(() => {
    const matchingObject =
      valueelemsec == "Elementery" ? exedirelemeduuserId : exedirseceduuserId;
    ...
    <>
      <ReactFullscreen>
        {({ onToggle }) => (
          <Layout
            pageTitle="Report"
            activeMenu="Initiator"
            reDirectUrl={"/initiator/initiatereportlist"}
            appId="PERSONNEL_ACTION_FORM"
            IsSuperadmin={personnelActionIsSuperadmin}
          >
            <div className="report-wrapper pt-24 md:pt-28 xl:pt-[2.083vw]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 xl:gap-[1.250vw] pb-5">
                ...
                <div
                  className="relative z-10 col-span-12 lg:col-span-12"
                  data-aos="fade-down"
                ...
                >
                  <div className="bg-[#fff] box-shadow-2 radius8 border border-[#E4E7EC]">
                    <div className="flex items-center justify-between p-[20px] xl:px-[1.250vw] xl:py-[0.990vw] border-b border-[#E4E7EC] ">
                      ...
                      <div className="flex gap-2 xl:gap-[0.990vw] items-center">
                        <>
                          {loggedUserRole == "Sidhant" ||
                          loggedUserRole == "POOJA" ? (
                            <button
                              onClick={() => setShowNextPopUp(true)}
                              className="w-full text-center tableBtn blue radius8"
                            >
                              <i className="gusd-pluse-circle mx-1.5 "></i>{" "}
                              Initiate New Report
                            </button>
                          ) : null}

                          <Psapopup
                            visible={showNextPopUp}
                            onHide={() => setShowNextPopUp(false)}
                            basicInfoPopUp={basicInfoPopUp}
                            partyDetailesPopup={partyDetailesPopup}
                            attchmentAndDetailes={attchmentAndDetailes}
                            termsAndConditions={termsAndConditions}
                            eddDetailes={eddDetailes}
                            previewAndEdit={previewAndEdit}
                            w9Detailes={w9Detailes}
                          />
                          <Link
                            href="#"
                            onClick={(e) => {
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12">
                        <div className="col-span-1 lg:col-span-12 bg-[#F5F6F7] h-auto">
                          <div className="p-5">
                            ...
                            <div
                              onClick={handlePrint}
                              className="bg-[#113699] border mt-10 border-[#D0D5DD] shadow-[0px_1px_2px_rgba(16,24,40,0.05)] font-medium text-[#FFF] flex items-center py-[10px] px-[8px] rounded-[8px] justify-center"
                            >
                              ...
                              <div style={{ display: "none" }}>
                                <div
                                  ...
                                  id="divname"
                                >
                                  <center className="font-bold text-[20px]">
                                    GLENDALE UNIFIED SCHOOL DISTRICT
                                  </center>
                  ...
                  <Sidebar
                    ...
                    position="right"
                    onHide={() => {
                      setinitiateReportssidebar(false);
                      setShowStatusPrint(false);
                    }}
                    style={{
                      width: "65vw",
                      minWidth: "40rem",
                      ...
                      <div className="col-span-1 lg:col-span-8 xl:py-[1.667vw] py-[28px] xl:px-[1.250vw] px-[20px] lg:space-y-[1.250vw] space-y-2 h-full">
                        <div className="relative h-full">
                          ...
                          <div>
                            ...
                            <div className="text-[#344054] font-normal text-xs xl:text-[0.677vw]">
                              Personnel Action
                            </div>
                          </div>
                          <div className="xl:h-[36.250vw] 2xl:h-[38.250vw] lg:h-[28.250vw] overflow-auto">
                            <div className="status-list">
                              <div className="flex justify-between">
                                <div className="text-[#344054] text-[14px]">
                                  Status
                                </div>
                                <div className="text-[#FF7F01] text-[12px] bg-[#FFF2E5] py-[4px] px-[8px] rounded-[18px]">
                                  <i className="gusd-dot text-[10px] mr-1"></i>{" "}
                                  Closed By HR Technician
                              ...
                              <div className="flex justify-between">
                                ...
                                <div className="text-[#344054] text-[14px] font-semibold">
                                  Certificated
                              ...
                              <div className="flex justify-between">
                                ...
                                <div className="text-[#344054] text-[14px] font-semibold">
                                  {apprpverdataEdit.personalReportNo}
                                </div>
                              </div>
                              <div className="flex justify-between">
                                <div className="text-[#344054] text-[14px]">
                              ...
                              <div className="flex justify-between">
                                ...
                                <div className="text-[#344054] text-[14px] font-semibold">
                                  {apprpverdataEdit.actionToBeTaken}
                                </div>
                              </div>
                            </div>
                            <div className="pt-3">
                              ...
                              <div className="py-3 emp-simple-tbl">
                                <DataTable
                                  ...
                                  value={
                                    specificReportDetailsView.approvalFinalData
                                  }
                                >
                                  <Column
                                    field="empCode"
                            ...
                            <div className=" bg-[#EFF0F1] mt-[24px] rounded-[8px] p-[8px]">
                              <div>
                                <div className="flex justify-between gap-2 text-[22px] text-[#000] items-center my-3">
                                  <div>
                                    <i className="pi pi-file"></i> Personnel
                                    Action.pdf
                                  </div>
                                  <div className="flex justify-end">
                                    <div
                                      onClick={() =>
                                        setinitiateReportsfullscreen(true)
                                      }
                                      className="text-[#113699] font-medium text-[14px] pb-3 cursor-pointer"
                                    >
                                      <i className="pi pi-calendar mr-1 mt-2"></i>
                                      View Full Screen{" "}
                                    </div>
                                  </div>
                                </div>
                                ...
                                >
                                  <div class="text-center mt-5">
                                    ...
                                    <div class="text-base text-black font-semibold">
                                      {" "}
                                      BOARD REPORT PERSONNEL ACTION{" "}
                                    </div>
                                  </div>
                                  <div class="flex justify-between my-3 ">
                                    <div class="m-2 text-sm">
                                      Location : {apprpverdataEdit.location}
                                      <br></br>
                                      Action to be Taken :{" "}
                                      {apprpverdataEdit.actionToBeTaken}
                                      <br></br>
                                      Type : {apprpverdataEdit.type}
                                      {apprpverdataEdit.actionItem ? (
                                        <div>
                                          Action Item :{" "}
                                          {apprpverdataEdit.actionItem}
                                          <br></br>
                                    ...
                                    <div class="m-2 text-sm">
                                      Board Meeting Date :{" "}
                                      {moment(
                                        apprpverdataEdit.boardMeetingDate
                                      ).format("MM/DD/yyyy")}{" "}
                                      <br></br>
                                      Personnel Report No :{" "}
                                      {apprpverdataEdit.personalReportNo}
                                      <br></br>
                                      {apprpverdataEdit.actionDate ? (
                                        <div>
                                          Action Date :{" "}
                                          {apprpverdataEdit.actionDate}
                                          <br></br>
                                    ...
                                    <table class="table border border-stone-900 text-xs">
                                      <thead>
                                        <tr class="border ">
                                          ...
                                          <th
                                            className="border border-[#000] px-2 py-2 text-xs"
                                            rowSpan={2}
                                            scope="col"
                                          >
                                            Emp Name
                                        ...
                                        {employeeArray?.map((item) => {
                                          return (
                                            ...
                                            {item.employeeId}
                                              ...
                                              <td className="border border-[#000] px-2 py-2">
                                                {item.positionTitleTo}
                                              </td>
                                              <td className="border border-[#000] px-2 py-2">
                                                {item.positionTitleFrom}
                                              </td>
                                              <td className="border border-[#000] px-2 py-2">
                                                {moment(
                                                  item.effectiveDatesFrom
                                                ).format("MM/DD/yyyy")}{" "}
                                                -{" "}
                                                {moment(
                                                  item.effectiveDatesTo
                                                ).format("MM/DD/yyyy")}
                                              </td>
                                              <td className="border border-[#000] px-2 py-2">
                                                {item.salaryRate}
                                  ...
                                  >
                                    <table
                                      class="table border border-stone-900 text-xs"
                                      style={{ width: "100%" }}
                                    >
                                      <thead style={{ width: "100%" }}>
                                        <tr class="border ">
                                          ...
                                          <th
                                            ...
                                            scope="col"
                                          >
                                            Effective Date, Salary Rate of
                                            Reason,
                                            <br />
                                            Not To Exceed..., Funding Source{" "}
                                            <br />
                                            And Account Number
                                          </th>
                                        </tr>
                                  ...
                                  >
                                    ...
                                    <table style={{ width: "100%" }}>
                                      ...
                                      <tbody>
                                        {specificReportDetailsView?.approvalFinalData?.map(
                                          (item) => {
                                            return (
                                              <tr className="text-center">
                                                <td
                        ...
                        <div className="absolute left-[20px] xl:left-[1.042vw] right-[20px] xl:right-[1.042vw] bottom-[20px] xl:bottom-[1.042vw]">
                          <div>
                            <div
                              onClick={() => {
                                setDisplay(true);
                              ...
                              className="text-[#FFF] font-medium text-[14px] xl:text-[0.729vw] text-center xl:py-[0.521vw] py-[8px] xl:px-[0.781vw] px-[14px] border border-[#D0D5DD] bg-[#113699] Shadow_xs rounded-lg space-x-2 cursor-pointer"
                            >
                    ...
                    <div className="xl:py-[1.667vw] py-[28px] xl:px-[1.250vw] px-[20px] lg:space-y-[1.250vw] space-y-2 h-full">
                      <div className="relative h-full">
                        ...
                        <div>
                          <div
                            className="cursor-pointer mb-[15px]"
                            onClick={() => setinitiateReportsfullscreen(false)}
                          >
                            <i className="gusd-close-sidebar text-[24px]"></i>
                          </div>
                          {/*col*/}
                              ...
                              <div>
                                <div className="flex justify-between gap-2 text-[22px] text-[#000] items-center my-3">
                                  ...
                                  <div className="flex justify-end">
                                    <div
                                      onClick={() =>
                                        setinitiateReportsfullscreen(false)
                                      }
                                      className="text-[#113699] font-medium text-[14px] pb-3 cursor-pointer"
                                    >
                                      <i className="pi pi-calendar mr-1 mt-2"></i>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-12">
                      <div className="col-span-1 lg:col-span-8 xl:py-[1.667vw] py-[28px] xl:px-[1.250vw] px-[20px] lg:space-y-[1.250vw] space-y-2 h-full">
                        <div className="">
                          ...
                          <div
                            className="cursor-pointer mb-[15px]"
                            onClick={() => setrequestedreportEdit(false)}
                          >
                            <i className="gusd-close-sidebar text-[24px]"></i>
                          </div>
                          {/*col*/}
                                ...
                                <div className="flex justify-between gap-2 text-[22px] text-[#000] items-center my-3">
                                  ...
                                  <div className="flex justify-end">
                                    ...
                                    <div
                                      style={{ padding: 8 }}
                                      onClick={() => {
                                        setEditPopupfromAP(true);
                                        setEditPopup(true);
                                      }}
                                      className="text-[#113699] font-medium text-[14px] pb-3 cursor-pointer"
                                    >
                                      <i className="gusd-edit mr-1 mt-2"></i>
                                      Edit{" "}
                                    </div>
                                    {/* : null
                                  } */}
                        ...
                        <div className="overflow-auto h-[300px]">
                          <div className="pl-5 pt-5 text-[#344054] font-medium text-[20px] flex">
                            <InputSwitch
                              ...
                              onChange={(e) => setAddSignature(e.value)}
                            />
                            <label style={{ marginLeft: "2%" }}>
                              Add Signature
                              <span class="text-[red] pl-0.2">*</span>
                            </label>
                          </div>
                        ...
                        <div>
                                    ...
                                    {genderss.map((gender, index) => (
                                      <div key={index} className="p-col-6 ">
                                        <RadioButton
                                          ...
                                          value={gender.value}
                                          onChange={(e) =>
                                            setValueelemsec(e.value)
                                          }
                                          checked={
                                            valueelemsec === gender.value
                                          }
                                          style={{ margin: 5 }}
                                        />
                          ...
                          /> */}
                            ...
                            apprpverdataEdit?.psayesorno == null ? (
                            <div
                  ...
                  >
                    <div className="xl:py-[1.667vw] py-[28px] xl:px-[1.250vw] px-[20px] lg:space-y-[1.250vw] space-y-2 h-full">
                      <div className="" style={{ height: "auto" }}>
                        ...
                        <div
                          className="cursor-pointer mb-[15px]"
                          onClick={() => setPayrollreportEditslider(false)}
                        >
                          <i className="gusd-close-sidebar text-[24px]"></i>
                        </div>
                        {/*col*/}
                                        ...
                                        return (
                                          ...
                                          <td className="border border-[#000] px-2 py-2">{item.accountNo}</td> */}
                                            <td className="border border-[#000] px-2 py-2">
                                              {/* {concatenateStringifiedArray(item.budgetCode)} */}
                                              {/* {item.budgetCode} */}
                                              {JSON.parse(item.budgetCode)?.map(
                                                (item, index) => (
                                                  <div key={index}>{item}</div>
                                                )
                                              )}
                  ...
                  >
                    <div className="m-0">
                      {sendMsg && (
                        <div className="font-medium text-[18px] text-[#249144] mb-6">
                          Message Sent Successfully
                        </div>
                      )}

                      <div className="font-medium text-[18px] mb-3">
