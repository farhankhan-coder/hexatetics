...
export default function Index() {
  ...
  const populatePopup = async (rowData) => {
    if (
      rowData.status !== AllStatusData.APPROVED &&
      rowData.status !== AllStatusData.CLOSED
    ) {
      setIsEdit(true);
    } else {
      setIsEdit(false);
    ...
    try {
      ...
      approverDateTime = moment(getAbsenceReport.data.approverDateTime).format(
        "MM/DD/YYYY HH:mm "
      );
      payrollDateTime = moment(getAbsenceReport.data.payrollDateTime).format(
        "MM/DD/YYYY HH:mm "
      );
      ...
      if (getAbsenceReport.data) {
        ...
        if (getAbsenceReport.data.userId) {
          ...
          let eventsTimeline = await getTimeline(
            getAbsenceReport.data.camundaProcessInstanceId,
            accessToken
          );
          ...
          for (var i = 0; i < eventsTimeline.length; i++) {
            ...
            obj.date = moment(eventsTimeline[i].date).format(
              "MM/DD/YYYY HH:mm"
            );
        ...
        if (getAbsenceReport.data.l2Authority) {
          ...
          let payrollname = empName.employee_code
            ? `${empName.employee_name} (${empName.employee_code})`
            : empName.employee_name;
        }
        ...
        setToDate(to_date);
        let from_date = moment(getAbsenceReport.data.fromDate).format(
          "YYYY-MM-DD"
        );
        ...
        for (var i = 0; i < getAbsenceReportOfEmployee.length; i++) {
          ...
          if (i === getAbsenceReportOfEmployee.length - 1) {
              ...
              if (newR[key] && newR[key].employeeResponse) {
                for (var k = 0; k < newR[key].employeeResponse.length; k++) {
                  ...
                  dataRec[countR].absentCode = {
                    name: `${newR[key].employeeResponse[k].absentCodeDetails.n} (${newR[key].employeeResponse[k].absentCodeDetails.t})`,
            ...
            let certificatedData = sortedArray.filter(
              (item) => item.employeeType === "Classified"
            );
    ...
    return (
      <>
        <div className="flex justify-center w-full gap-2">
          ...
          rowData.status !== AllStatusData.CLOSED ? (
            <Link
              href={`/war_classified/initiate/${rowData.id}`}
  ...
  const loadAbsenceReport = async (pageNumber, row) => {
    ...
    try {
      const response = await axios.post(
        "/api/war_classified/war_classified_listing",
        { requestedData }
      );
    ...
    if (weeklyAbsenceReport.length > 0) {
      for (let i = 0; i < weeklyAbsenceReport.length; i++) {
        ...
        else if (weeklyAbsenceReport[i].status === AllStatusData.PENDING) {
          status = AllStatusData.PENDING; remark = "-";
        }
        else if (weeklyAbsenceReport[i].status === AllStatusData.SUBMITTED) {
          status = AllStatusData.SUBMITTED; remark = "-";
        }
        else if (weeklyAbsenceReport[i].status === AllStatusData.REVIEWED_AND_RESUBMITTED) {
          status = AllStatusData.REVIEWED_AND_RESUBMITTED; remark = "-";
        }


        newResponse.push({
          "id": weeklyAbsenceReport[i].id,
          "userId": weeklyAbsenceReport[i].user_id, "status": status, "schoolId": weeklyAbsenceReport[i].school_id, "fromDate": weeklyAbsenceReport[i].from_date,
          "toDate": weeklyAbsenceReport[i].to_date, "statusDateTime": weeklyAbsenceReport[i].statusDateTime, "approverStatus": weeklyAbsenceReport[i].approverStatus, "approver_by": weeklyAbsenceReport[i].approver_by, "approver_remark": weeklyAbsenceReport[i].approver_remark, "approver_date_time": weeklyAbsenceReport[i].approver_date_time,
        ...
        <div className="report-wrapper pt-24 md:pt-28 xl:pt-[2.083vw] mb-10">
          <FullScreen handle={handle}>
            <WARCertificatedReports
              ...
              submittedFilterVlaue={submittedFilterVlaue}
