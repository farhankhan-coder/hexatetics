...
import WARClassifiedReports from '@/pages/war_classified/report';
...
export default function Index() {
  ...
  const populatePopup = async (rowData) => {
    ...
    if (rowData.status === AllStatusData.PENDING_FOR_ACKNOWLEDGEMENT) {
      setIsEdit(true);
    } else {
      setIsEdit(false);
    }
    ...
    return (
      <>
        <div className="flex justify-center w-full gap-2">
          {
            rowData.status === AllStatusData.PENDING_FOR_ACKNOWLEDGEMENT ?
              <Link
                href="#"
  ...
  const loadAbsenceReport = async (pageNumber, row) => {
    ...
    try {
      const response = await axios.post("/api/war_classified/war_classified_listing", { requestedData });
    ...
    if (weeklyAbsenceReport.length > 0) {
      for (let i = 0; i < weeklyAbsenceReport.length; i++) {
        ...
        if (weeklyAbsenceReport[i].status === AllStatusData.APPROVED) {
          status = AllStatusData.PENDING_FOR_ACKNOWLEDGEMENT; remark = "-";
        }
    ...
    <>
      <Layout pageTitle="WAR Certificated-Admin" activeMenu="Initiator">
        <div className='report-wrapper pt-24 md:pt-28 xl:pt-[2.083vw] mb-10'>
            <WARClassifiedReports weeklyAbsenceReports={weeklyAbsenceReports} isListLoaded={isListLoaded} onPage={onPage} count={count} weekFilterValue={weekFilterValue} schoolFilterValue={schoolFilterValue} submittedFilterVlaue={submittedFilterVlaue} statusFilterValue={statusFilterValue} setWeekFilter={setWeekFilter} setSchoolFilter={setSchoolFilter} setSubmittedFilter={setSubmittedFilter} setStatusFilter={setStatusFilter} reportType='Requested Reports' lazyState={lazyState} rows={rows} activeIndex={2} reportListActions={reportListActions}></WARClassifiedReports>
        </div>
        <EyePopup
          visible={visible}
