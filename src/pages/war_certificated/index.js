...
export default function Index() {
  ...
  const populatePopup = async (rowData) => {
    if (rowData.status !== AllStatusData.APPROVED && rowData.status !== AllStatusData.CLOSED) {
      setIsEdit(true);
    } else {
      setIsEdit(false);
    }
    ...
    return (
      <>
        <div className="flex justify-center w-full gap-2">
          {
            rowData.status !== AllStatusData.APPROVED && rowData.status !== AllStatusData.CLOSED ?
              <Link
                href={`/war_certificated/initiate/${rowData.id}`}
  ...
  const handelWeekDurationChange = () => {
    return (
      <InputText value={weekFilterValue} onChange={(e) => setWeekFilter(e.target.value)} placeholder="Select" className="p-column-filter custDropdown" showClear style={{ minWidth: '7rem' }} />
    );
  }

  const handelSchoolNameChange = () => {
    return (
      <InputText value={schoolFilterValue} onChange={(e) => setSchoolFilter(e.target.value)} placeholder="Select" className="p-column-filter custDropdown" showClear style={{ minWidth: '7rem' }} />
    );
  };

  const handelSubmittedOnChange = () => {
    return (
      <InputText value={submittedFilterVlaue} onChange={(e) => setSubmittedFilter(e.target.value)} placeholder="Select" className="p-column-filter custDropdown" showClear style={{ minWidth: '7rem' }} />
    );
  }
